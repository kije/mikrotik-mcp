...
`print_resource` takes the same optional `device` (inventory title) as every
tool and forwards it to `execute_mikrotik_command`; a connection or
device-resolution error is returned verbatim rather than parsed into an empty
record list. Each device-backed resource is registered twice: the plain URI
(e.g. `mikrotik://ip/address`) reads the default device and is valid when the
inventory holds exactly one, while the template
`mikrotik://device/{device}/ip/address` reads the named device.
