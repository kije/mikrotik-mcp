...
4. **Run with SSE or streamable HTTP transport:**
   ...
   **Environment Variables:**
   ...
   | `MIKROTIK_INVENTORY` | Multiple devices, written inline in YAML (or its JSON subset). Takes precedence over `MIKROTIK_INVENTORY_FILE` and the single-device variables above. See [Inventory](../reference/inventory/README.md). | _(empty)_ |
