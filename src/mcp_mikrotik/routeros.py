...
async def print_resource(
    ...
    *,
    ...
    """Run a ``print`` on ``device`` and return it in the requested shape.
    ...
    * ``raw``    — a plain ``print`` exactly as before this helper existed.
    ...
    # Connection / device-resolution failures must reach the caller verbatim,
    # not be parsed into an empty record list.
