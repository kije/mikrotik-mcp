...
def test_print_resource_forwards_device(monkeypatch):
    seen = {}
    ...
    monkeypatch.setattr(routeros, "execute_mikrotik_command", fake, raising=True)
    asyncio.run(print_resource(None, "/ip address", device="RouterB"))
    assert seen["device"] == "RouterB"


def test_print_resource_json_surfaces_errors(monkeypatch):
    """An error (e.g. an unknown device) must not be parsed into zero records."""
    error = "Error: Unknown device 'nope'. Available devices: RouterA."
